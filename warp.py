...
def export_Hiddify(t_ips, f_ips):
    ...
    config_prefix = f'warp://{t_ips[0]}?ifp=10-20&ifps=20-60&ifpd=5-10#WARP-ONE&&detour=warp://{t_ips[1]}?ifp=10-20&ifps=20-60&ifpd=5-10#WARP-TWO'

    title = "//profile-title: base64:" + base64.b64encode('WARP (by \'Aksi)'.encode('utf-8')).decode(
        'utf-8') + "\n"
    update_interval = "//profile-update-interval: 12\n"
    sub_info = "//subscription-userinfo: upload=0; download=0; total=10737418240000000; expire=2546249531\n"
    profile_web = "//profile-web-page-url: https://github.com/AksiSpirit/cf-warp\n"
...
def main(script_dir):
    arch = arch_suffix()
    bin_name = f"./yxip/warp-linux-{arch}"
    subprocess.run(["cp", bin_name, "./warp"])
    os.chmod("warp", 0o755)
